...
freq_table <- function(var.cat, y, data,
                       margin = NULL,
                       nround = 2,
                       latex = TRUE,
                       sz.tab = "footnotesize",
  ...
  for (i in 1:length(var.cat)) {
    ...
    res_freq <- matrix(paste0(freq_abs," (", round(freq_rel,nround),"%)" ), ncol = ncol(freq_abs))
    res_freq <- cbind(c(var.cat[i],rep("",nrow(res_freq) - 1)),rownames(freq_abs), res_freq   )
    # rownames(res_freq) <- rownames(freq_abs)
    colnames(res_freq) <- c("Variable", "Levels", colnames(freq_abs))
    res_freq_all <- rbind(res_freq_all, rep("", ncol(res_freq) ),res_freq)
  }
  if (latex) {stargazer(res_freq_all,summary = F,type = "latex",add.lines = c(1,2,3),
